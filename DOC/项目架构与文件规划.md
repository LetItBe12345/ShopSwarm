...
日期：2026-09-23。状态：架构草案；M0.2 已建立最小包、插件和诊断工具，其余目录按 roadmap 逐步创建。
...
本稿引用的本地副本不代表上游最新接口。历史文档对性能、Profile、登录复用和焦点行为的描述不作为项目保证。M0.1 已固定首个验证环境、版本和测试边界；M0.2 已验证项目内的插件注册、工具调用和无登录浏览器连通。独立 DSH profile、取消和卸载由 M0.3 验证，Profile 和真实购物站点仍需按 M1、M2 实测。
