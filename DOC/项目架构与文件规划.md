...
日期：2026-09-23。状态：架构草案；M0—M2、M3.1 已实现并有验证记录。当前产品范围是通用自然语言购物研究；Coding Plan / LLM 中转套餐保留为领域 fixture。以下文件树同时包含当前实现和后续建议项。

本稿依据 roadmap 和本地参考副本编写。文件树同时标出已有文件和后续规划，规划项不代表已有实现。

## 0. 当前产品方向

浏览器和 Agent 架构保持领域无关。用户入口是自然语言购物意图，DSH / Lead Agent 负责拆成来源子任务；当前代码中的 `ShoppingTaskContext`、`Offer` 和 `src/shopping/` 是 M1/M2 已实现兼容接口，不立即重命名。M4 再新增通用任务、报价、领域扩展和确定性归一化层。

当前产品规则以 [通用购物任务决策](../decision/产品范围-通用购物任务.md) 和 [真实购物任务与测试策略](真实购物任务与测试策略.md) 为准。[Coding Plan 比价规则](Coding%20Plan%20比价规则.md) 只定义一个复杂领域 fixture。
...
│   ├── Coding Plan 比价规则.md             [现有] Coding Plan 领域 fixture 的价格语义、额度、证据与可比性规则
...
│   ├── domains/                           [规划 M4] 可选领域扩展；Coding Plan 只是其中一个 fixture
...
│   ├── live-shopping.e2e.test.ts           显式启用的多领域真实购物来源验证
│   └── fixtures/
...
- `Task`：保留原始自然语言购物目标、来源范围、硬约束、偏好和完成条件。M4 区分 requiredFields 与 desiredFields。M2 暂不设置任务级执行预算。
...
例如任务可以是“找一台 27 英寸 2K 显示器并比较公开来源”，也可以是“核对某 Coding Plan 的月费和额度”。Jev 只在单个页面内选择下一步导航动作；每个动作后重新读取页面。Jev 选择 DONE 后，Verifier 仍要核对任务的 required 字段。desired 字段未公开时保持 unknown；required 字段缺失则该来源不能成功。
