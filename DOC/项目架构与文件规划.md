...
日期：2026-09-23。状态：设计稿，供讨论；源码目录、工具名称和接口尚未确认，也尚未实现。

本稿依据现有 roadmap 和本地参考副本编写。现有工程仍处于设计阶段。这次只新增本文与绘图产物，不创建规划中的源码目录，不改变 roadmap 完成状态。
...
├── README.md                              [规划 M0/M5] 项目入口、安装与文档链接
├── package.json                           [规划 M0] 包入口、脚本、dsh.bundle.patch
├── cordis.patch.yml                       [规划 M0] 将 Host Plugin 加入 DSH profile
├── tsconfig.json                          [规划 M0] TypeScript 编译配置
├── <包管理器锁文件>                        [规划 M0] 确认包管理器后确定名称
├── .gitignore                             [规划 M0] 排除构建产物、凭据与运行数据
...
│   ├── 兼容性与支持范围.md                 [规划 M0—M5] 实测版本、平台、站点与限制
│   ├── 安装与使用.md                       [规划 M0/M5] 安装、配置、首个任务、移除
...
│
├── src/                                   [以下均为规划，不在本次创建]
│   ├── index.ts                           M0：apply(ctx)，注册工具及释放处理
│   ├── config.ts                          M0：配置解析、预算与运行数据路径
...
这是首版完成时的建议结构，不是 M0 要一次性建立的目录树。M0 只需包配置、插件入口、最小工具和连通验证。模块较小时可以先放在一个文件，职责出现后再拆分。暂不设 MCP、Browser Use、浏览器池、独立 UI 或自建 Agent 调度器目录。

编译后的 `dist/` 是生成物。真实证据、日志、临时 Profile 与凭据放在用户配置的仓库外运行目录；具体路径在 M0 确定。示例数据经过脱敏后才能放入 fixtures。
...
本稿仅核对本地副本，不代表上游最新接口。历史文档对性能、Profile、登录复用和焦点行为的描述不作为项目保证。具体运行平台、站点、品类、依赖版本、工具契约及清理钩子均需按 M0/M1 验证。
