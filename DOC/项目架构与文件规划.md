...
日期：2026-09-23。状态：架构草案；M0—M2.2 已按任务逐步实现，以下文件树仍包含未建立的建议项。
...
│   ├── shopping/
│   │   ├── run.ts                         [现有 M2.2] 单站动作循环、重开页面及结构化结果
│   │   ├── extract.ts                     [现有 M2.2] 从页面证据形成候选报价
│   │   ├── verify.ts                      [现有 M2.2] 独立完成检查、缺失字段与不匹配原因
│   │   ├── match.ts                       M4：按任务核对型号、规格或计费方式，不合并不同身份
...
- `Task`：商品目标、站点范围、规格约束和完成条件。M2 暂不设置任务级执行预算。
