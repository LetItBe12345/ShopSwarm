...
本稿仅核对本地副本，不代表上游最新接口。历史文档对性能、Profile、登录复用和焦点行为的描述不作为项目保证。M0.1 已在 [兼容性与支持范围](兼容性与支持范围.md) 中固定首个验证环境、版本和测试边界，并核对 DSH 工具接口；插件运行、浏览器行为、Profile 和真实站点仍需按 M0.2、M0.3、M1 和 M2 实测。
