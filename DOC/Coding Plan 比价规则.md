...
日期：2026-09-23。状态：Coding Plan 领域测试与数据语义草案。它是通用 ShopSwarm 的一个复杂领域 fixture，不再作为 M2.4—M5 的整体产品基线。当前代码仍保留 M1/M2 的购物命名，本文中的新结构不代表已经实现。

## 1. 领域定位

Coding Plan、LLM API 套餐和第三方中转套餐是 ShopSwarm 的一个测试域。它适合验证套餐身份、价格类型、额度单位、时间窗口、倍率和 unknown 等复杂语义，但不能代表 ShopSwarm 的全部购物任务。

当前产品目标是通用自然语言购物研究。Coding Plan 来源仍优先使用公开价格页、套餐页或公开接口，只读取和比较信息，不购买、不充值、不支付，也不以破解验证码或绕过站点风控作为验收条件。
...
3. Coding Plan 真实公开来源：至少两个 Coding Plan 或中转套餐来源，保存全部成功与失败尝试。它只满足通用 E2E 矩阵中的“数字服务 / Coding Plan”一行。
...
M2.4 当时的范围决策见 [M2.4 历史决策](../decision/M2.4-阶段验收.md)。当前产品范围见 [通用购物任务决策](../decision/产品范围-通用购物任务.md)，完整多领域测试矩阵见 [真实购物任务与测试策略](真实购物任务与测试策略.md)。
