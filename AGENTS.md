...
- 设计记录提出了 DSH 集成、浏览器运行、Jev 动作循环、并行比较、价格核对和发布六个阶段。制定任务时应核对前置条件，不把阶段建议当成已完成工作。首个真实领域是 Coding Plan、LLM API 套餐和第三方中转套餐；底层任务保持通用。GLM-5.3 只作为固定测试用例。商城页面不再作为默认验收主线。
...
- Coding Plan 比较至少核对 provider、plan、模型/模型集合、billing mode/period、price type、quota 单位/窗口、条件规则、来源和采集时间。未知价格、额度、缓存价、倍率和 eligibility 保持 unknown。不同语义默认不强行排序。
...
- 完成任务、写完代码并通过与改动相匹配的充分本地测试后，主动提交、推送任务分支并创建目标为 `main` 的 PR；不必等用户再次提出。创建 PR 后立即用 `gh pr merge --auto --squash` 登记自动合并，确认 GitHub 已接受登记。`main` 的必需检查是 CI 的 `offline`；检查通过后由 GitHub 合并。CI 失败时修复问题并更新同一个 PR，不绕过检查。登记成功后即可结束本次 Agent 运行，不必等待 GitHub Actions；当时的检查与合并状态要如实说明。
...
- 当前产品范围见 `decision/M2.4-阶段验收.md`、`DOC/Coding Plan 比价规则.md` 和 `roadmap.md`。`DOC/购物比价项目设计.md` 只保留历史设计，其中的商城方案已停止作为默认主线。
- Jev 上下文、动态动作集合、元素引用和执行层设计见 `DOC/分支-购物比价项目设计.md`。
- Jev 一次调用的 `state`、`questions` 和多轮状态归属见 `DOC/Jev调用与状态.md`。其中示例不是已对接的 HTTP 契约。
- ShopSwarm 面向多 Agent 并行价格研究。首个真实领域是 Coding Plan / LLM 中转套餐，但浏览器、Jev 和并行框架不写死具体模型或提供方。当前设计方向仍是 DSH 外部 Bundle + Host Plugin + Skill，不修改 DSH 核心。
...
- Jev 只负责当前有效 Action Space 内的动作选择。字段提取后由 Verifier 核对；价格计算、额度换算、comparability 和排序使用确定性代码。
