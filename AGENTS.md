...
- 完成任务、写完代码并通过与改动相匹配的充分本地测试后，主动提交、推送任务分支并创建目标为 `main` 的 PR；不必等用户再次提出。创建 PR 后即可结束本次 Agent 运行，不等待 GitHub Actions 结束；当时的检查状态要如实说明。
- 合并 PR、创建 Tag 和发布 Release 仍只在用户明确要求时执行。创建 PR 不代表已获合并或发布授权。
