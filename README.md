...
ShopSwarm 是一个面向多 Agent 并行购物研究与比价的项目。M0 和 M1 阶段已通过，M2 的单站动作循环和本地页面复核已实现；真实店铺验证仍在开发。
...
- 已完成 M2.2 的单站 Jev 动作循环、独立完成检查、重开页面复核，以及登录、验证码、超时和无进展的结构化返回。本地页面使用真实 Jev、DeepSeek 和浏览器跑通；真实店铺尚未验证。
- 下一项是 M2.3：验证单站真实任务，并建立公平的性能基线。
